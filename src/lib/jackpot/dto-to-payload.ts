...
export function dtoToPayload(dto: JackpotDTO): JackpotSavePayload {
  ...
  return {
    ...
    minWagerAmount: 0,
    maxWagerAmount: 0,
    reseedingAmount: Number(seed.minimumSeedAmount ?? seedAmount),
    minimumSeedAmount: Number(seed.minimumSeedAmount ?? seedAmount),
    maximumSeedAmount: Number(seed.maximumSeedAmount ?? 0),
