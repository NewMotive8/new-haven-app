...
import { WheelRuleInstanceDTO } from '../ruleInstace';

export type StateT = 'ACTIVE' | 'EXPIRED' | 'CANCELLED';

export interface RuleProgressDTO {
  id: number;
  wheelRuleInstance: WheelRuleInstanceDTO | null;
  player: playersI | null;
  progress: number;
  state: StateT;
  createdDate: string;
  updatedDate: string;
...
export const defaultRuleProgress: RuleProgressDTO = {
  id: 0,
  wheelRuleInstance: null,
  player: null,
  progress: 0,
  state: 'ACTIVE',
  createdDate: '',
...
async function getAll(params?: pageableProps) {
  return api
    .get(urls.luckWheel.ruleProgress.getAll, { params })
...
function sanitizePayload(payload: RuleProgressDTO): RuleProgressDTO {
...
async function handleConfirmDelete({
  ...
  await api
    .delete(`${urls.luckWheel.ruleProgress.delete}/${id}`)
    .then((res) => {
      if (successCallBack) {
        successCallBack(res);
      }
...
export const LWRuleProgressApi = {
  getAll,
  submitForm,
  defaultRuleProgress,
  deleteItem,
