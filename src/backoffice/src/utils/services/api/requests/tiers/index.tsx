...
const defaultItem: tiersI = {
    id: 0,
    // Default values for your entity properties.
}

function getItems(params?: pageableProps) {
    return api.get(urls.tiers.getAll, { params }).then((res) => ({ content: res.data }))
}
...
async function handleConfirmDelete({ id, successCallBack, errorCallBack }: deleteItemI) {
    await api.delete(`${urls.tiers.delete}/${id}`).then((res) => {
        if (successCallBack) {
            successCallBack(res)
        }
...
async function submitForm(formData: tiersI, options?: submitFormOptions) {
    const { errorCallBack, successCallBack, silent } = options || {}
    const action = formData?.id ? api.put : api.post
    const url = formData?.id ? `${urls.tiers.update}/${formData.id}` : urls.tiers.create
