...
interface SegmentI { // TODO: move this interface to appropriate service once it done.
    id: number;
    name: string;
    description: string;
    brand: brandI;
...
async function getItems(params?: pageableProps) {
    return api.get(urls.players.getAll, { params }).then((res) => ({ content: res.data }))
}
...
async function handleConfirmDelete({ id, successCallBack, errorCallBack }: deleteItemI) {
    await api.delete(`${urls.players.delete}/${id}`).then((res) => {
        if (successCallBack) {
            successCallBack(res)
        }
...
async function submitForm(formData: playersI, options?: submitFormOptions) {
    const { errorCallBack, successCallBack, silent } = options || {}
    const action = formData?.id ? api.put : api.post
    const url = formData?.id ? `${urls.players.update}/${formData.id}` : urls.players.create
