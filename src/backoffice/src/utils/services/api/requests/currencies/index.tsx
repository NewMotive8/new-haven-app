...
async function getItems(params?: pageableProps) {
  return api
    .get(urls.currencies.getAll, { params })
    .then((res) => (res.data))
}
...
async function handleConfirmDelete({
  ...
  await api
    .delete(`${urls.currencies.delete}/${id}`)
    .then((res) => {
      if (successCallBack) {
        successCallBack(res)
      }
...
async function submitForm(formData: currenciesI, options?: submitFormOptions) {
  const { errorCallBack, successCallBack, silent } = options || {}
  const action = formData?.id ? api.put : api.post
  const url = formData?.id
    ? `${urls.currencies.update}/${formData.id}`
    : urls.currencies.create
