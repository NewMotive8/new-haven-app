...
async function getItems(params?: pageableProps) {
  return api
    .get(urls.properties.getAll, { params })
...
async function handleConfirmDelete({
  ...
  await api
    .delete(`${urls.properties.delete}/${id}`)
    .then((res) => {
      if (successCallBack) {
        successCallBack(res)
      }
...
async function submitForm(formData: propertiesI, options?: submitFormOptions) {
  const { errorCallBack, successCallBack, silent } = options || {}
  const action = formData?.id ? api.put : api.post
  const url = formData?.id
    ? `${urls.properties.update}/${formData.id}`
    : urls.properties.create
