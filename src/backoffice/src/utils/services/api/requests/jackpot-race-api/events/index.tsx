...
function getItems(params?: pageableProps) {
    return api.get(urls.JR.events.getAll, { params }).then((res) => ({ content: res.data }))
}
async function eventsPerJackpot(spinSprintId: number) {
    const res = await api.get(urls.JR.events.eventsPerJackpot.replace('{{spinSprintId}}', `${spinSprintId}`))
...
async function bindEventToJackpotRace({ spinSprintId, eventId }: bindUnbind) {
    return api.get(urls.JR.events.bindToJackpotRace
        .replace('{{spinSprintId}}', `${spinSprintId}`)
        .replace('{{eventId}}', `${eventId}`))
        .then((res) => ({ content: res.data }))
}
async function unbindEventToJackpotRace({ spinSprintId, eventId }: bindUnbind) {
    return api.get(urls.JR.events.unbindToJackpotRace
        .replace('{{spinSprintId}}', `${spinSprintId}`)
        .replace('{{eventId}}', `${eventId}`))
        .then((res) => ({ content: res.data }))
}
...
async function handleConfirmDelete({ id, successCallBack, errorCallBack }: deleteItemI) {
    await api.delete(`${urls.JR.events.delete}/${id}`).then((res) => {
        if (successCallBack) {
            successCallBack(res)
        }
...
async function submitForm(formData: eventsI, options?: submitFormOptions) {
    ...
    const url = formData?.id ? `${urls.JR.events.update}/${formData.id}` : urls.JR.events.create
...
const eventsApi = {
    ...
    getItems,
    eventsPerJackpot,
    bindEventToJackpotRace,
    unbindEventToJackpotRace,
}
