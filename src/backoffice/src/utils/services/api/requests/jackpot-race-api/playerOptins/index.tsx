...
async function getByPlayer({ playerId, ...params }: GetByPlayerPros) {
  return api
    .get(`${urls.JR.playerOptins.getAll}/player/${playerId}`, { params })
    .then((res) => ({ content: res.data }))
}

async function getItems(params?: pageableProps) {
  return api
    .get(urls.JR.jackpotRace.getAll, { params })
    .then((res) => ({ content: res.data }))
}
...
async function handleConfirmDelete({
  ...
  // await api.delete(`${urls.JR.playerOptins.delete}/${id}`).then((res) => {
  //     if (successCallBack) {
  //         successCallBack(res)
  //     }
...
async function submitForm(
  ...
  const url = formData?.id ? `${urls.JR.playerOptins.update}/${formData.id}` : urls.JR.playerOptins.create
