...
const defaultItem: playerWinsI = {
    id: undefined,
    playerId: 0,
    spinSprintWin: {} as winI,
    brandId: 0,
    createdDate: '',
    payoutAmount: 0,
    betTime: '',
...
async function getItems(params?: pageableProps) {
    return api.get(urls.JR.playerWins.getAll, { params }).then((res) => (res.data))
}


async function exportCsvByEmail(brandId: number, params?: Pick<pageableProps, 'filterExp' | 'sort'>) {
  return api.get(urls.JR.playerWins.csv, {
    headers: { brandId },
    params,
  })
}
...
async function handleConfirmDelete({ id, successCallBack, errorCallBack }: deleteItemI) {
    await api.delete(`${urls.JR.playerWins.delete}/${id}`).then((res) => {
        if (successCallBack) {
            successCallBack(res)
        }
...
async function submitForm(formData: playerWinsI , options?: submitFormOptions) {
    const { errorCallBack, successCallBack, silent } = options || {}
    const action = formData?.id ? api.put : api.post;
    const url = formData?.id ? `${urls.JR.playerWins.update}/${formData.id}` : urls.JR.playerWins.create;
