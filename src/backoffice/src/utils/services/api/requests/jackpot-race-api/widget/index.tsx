...
/* =======================
   GET BY SPINSPRINT
======================= */
export async function getBySpinSprint(spinSprintId: number) {
  return api
    .get(urls.JR.widget.getBySprint.replace('{{spinSprintId}}', String(spinSprintId)))
    .then(res => res.data)
}
...
async function handleConfirmDelete({ id, successCallBack, errorCallBack }: deleteItemI) {
  await api
    .delete(`${urls.JR.widget.delete}/${id}`)
    .then(res => {
      successCallBack?.(res)
...
export async function submitForm(formData: widgetI, options?: submitFormOptions) {
  ...
  if (!formData.spinSprint?.id) {
    toastError('SpinSprint ID is missing')
    errorCallBack?.()
    return
  }

  const action = formData.id ? api.put : api.post
  const url = formData.id ? `${urls.JR.widget.update}/${formData.id}` : urls.JR.widget.create
