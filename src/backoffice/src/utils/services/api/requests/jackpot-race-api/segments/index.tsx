...
async function getItems(params?: pageableProps) {
    return api.get(urls.JR.segments.getAll, { params }).then((res) => ({ content: res.data }))
}

async function segmentsPerJackpotRace(spinSprintId: number) {
    const res = await api.get(urls.JR.segments.segmentsPerJackpotRace.replace('{{spinSprintId}}', `${spinSprintId}`))
    return (res?.data?.content || res?.data || [])
}
async function segmentsPerPlayer(playerId: number) {
    const res = await api.get(urls.JR.segments.segmentsPerPlayer.replace('{{playerId}}', `${playerId}`))
    return (res?.data?.content || res?.data || [])
}
...
async function bindSegmentToJackpot({ spinSprintId, segmentId }: bindUnbind) {
    return api.get(urls.JR.segments.bind
        .replace('{{spinSprintId}}', `${spinSprintId}`)
        .replace('{{segmentId}}', `${segmentId}`))
        .then((res) => ({ content: res.data }))
}
async function unbindSegmentToJackpot({ spinSprintId, segmentId }: bindUnbind) {
    return api.get(urls.JR.segments.unbind
        .replace('{{spinSprintId}}', `${spinSprintId}`)
        .replace('{{segmentId}}', `${segmentId}`))
        .then((res) => ({ content: res.data }))
}
...
async function bindSegmentToPlayer({ playerId, segmentId }: bindUnbindPlayer) {
    return api.get(urls.JR.segments.bindPlayer
        .replace('{{playerId}}', `${playerId}`)
        .replace('{{segmentId}}', `${segmentId}`))
        .then((res) => ({ content: res.data }))
}
async function unbindSegmentToPlayer({ playerId, segmentId }: bindUnbindPlayer) {
    return api.get(urls.JR.segments.unbindPlayer
        .replace('{{playerId}}', `${playerId}`)
        .replace('{{segmentId}}', `${segmentId}`))
        .then((res) => ({ content: res.data }))
}
...
async function handleConfirmDelete({ id, successCallBack, errorCallBack }: deleteItemI) {
    await api.delete(`${urls.JR.segments.delete}/${id}`).then((res) => {
        if (successCallBack) {
            successCallBack(res)
        }
...
async function submitForm(formData: segmentsI, options?: submitFormOptions) {
    ...
    const url = formData?.id ? `${urls.JR.segments.update}/${formData.id}` : urls.JR.segments.create
