...
const defaultItem: winI = {
    ...
    instantWin: '',
    status: '',
    jackpotRaceInstanceId: 0,
    players: [],
    createdDate: '',
};

async function getItems(params?: pageableProps) {
    return api.get(urls.JR.win.getAll, { params }).then((res) => res.data)
}
...
async function handleConfirmDelete({ id, successCallBack, errorCallBack }: deleteItemI) {
    await api.delete(`${urls.JR.win.delete}/${id}`).then((res) => {
        if (successCallBack) {
            successCallBack(res)
        }
...
async function submitForm(formData: winI , options?: submitFormOptions) {
    ...
    const url = formData?.id ? `${urls.JR.win.update}/${formData.id}` : urls.JR.win.create;
