...
import { jackpotRaceI } from '../jackpotRace'

export interface payoutI {
    id?: number | null | undefined
    spinSprint: jackpotRaceI | undefined
    startPlace: number
    endPlace: number
    amount: number
}

const defaultPayout: payoutI = {
    id: undefined,
    spinSprint: undefined,
    startPlace: 0,
    endPlace: 0,
    amount: 0,
}

async function getPayouts(params?: pageableProps) {
    return api.get(urls.JR.payout.getAll, { params }).then((res) => res.data)
}

async function getPayoutByRace(raceId: number) {
    return api.get(urls.JR.payout.getByRace.replace('{{raceId}}', `${raceId}`)).then((res) => res.data)
}

interface deletePayoutI {
    id: number | string
    successCallBack?: Function
    errorCallBack?: Function
}

async function handleConfirmDeletePayout({ id, successCallBack, errorCallBack }: deletePayoutI) {
    await api.delete(`${urls.JR.payout.delete}/${id}`).then((res) => {
        if (successCallBack) {
            successCallBack(res)
        }
...
async function deletePayout(props: deletePayoutI) {
                ...
                <Grid horizontalAlgin="space-between">
                    ...
                    <Grid width={45}>
                        <Button
                            ...
                            type="button"
                            block
                            onClick={() => handleConfirmDeletePayout(props)}
                            color="primary"
                        >
                            <Typography
                                translateGroup="global"
...
async function submitPayoutForm(formData: payoutI, options?: submitFormOptions) {
    const { errorCallBack, successCallBack, silent } = options || {}
    const action = formData?.id ? api.put : api.post
    const url = formData?.id ? `${urls.JR.payout.update}/${formData.id}` : urls.JR.payout.create
