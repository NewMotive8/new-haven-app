...
function getItems(params?: pageableProps) {
    return api.get(urls.events.getAll, { params }).then((res) => ({ content: res.data }))
}
async function eventsPerJackpot(jackpotId: number) {
    const res = await api.get(urls.events.eventsPerJackpot.replace('{{jackpotId}}', `${jackpotId}`))
...
async function bindEventToJackpot({ jackpotId, eventId }: bindUnbind) {
    return api.get(urls.events.bindToJackpot
        .replace('{{jackpotId}}', `${jackpotId}`)
        .replace('{{eventId}}', `${eventId}`))
        .then((res) => ({ content: res.data }))
}
async function unbindEventToJackpot({ jackpotId, eventId }: bindUnbind) {
    return api.get(urls.events.unbindToJackpot
        .replace('{{jackpotId}}', `${jackpotId}`)
        .replace('{{eventId}}', `${eventId}`))
        .then((res) => ({ content: res.data }))
}
...
async function handleConfirmDelete({ id, successCallBack, errorCallBack }: deleteItemI) {
    await api.delete(`${urls.events.delete}/${id}`).then((res) => {
        if (successCallBack) {
            successCallBack(res)
        }
...
async function submitForm(formData: eventsI, options?: submitFormOptions) {
    ...
    const url = formData?.id ? `${urls.events.update}/${formData.id}` : urls.events.create
