...
import { raffleI } from '../raffle'

export interface payoutI {
    id?: number | null | undefined
    raffle: raffleI | undefined
    startPlace: number
    endPlace: number
    amount: number
}
...
async function getPayouts(params?: pageableProps) {
    return api.get(urls.RF.payout.getAll, { params }).then((res) => res.data)
}

async function getPayoutByRace(raceId: number) {
    return api.get(urls.RF.payout.getByRace.replace('{{raceId}}', `${raceId}`)).then((res) => res.data)
}
...
async function handleConfirmDeletePayout({ id, successCallBack, errorCallBack }: deletePayoutI) {
    await api.delete(`${urls.RF.payout.delete}/${id}`).then((res) => {
        if (successCallBack) {
            successCallBack(res)
        }
...
async function submitPayoutForm(formData: payoutI, options?: submitFormOptions) {
    ...
    const url = formData?.id ? `${urls.RF.payout.update}/${formData.id}` : urls.RF.payout.create
