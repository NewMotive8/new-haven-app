...
async function getItems(params?: pageableProps) {
    return api.get(urls.RF.playerWins.getAll, { params }).then((res) => (res.data))
}


async function exportCsvByEmail(brandId: number, params?: Pick<pageableProps, 'filterExp' | 'sort'>) {
  return api.get(urls.RF.playerWins.csv, {
    headers: { brandId },
    params,
  })
}
...
async function handleConfirmDelete({ id, successCallBack, errorCallBack }: deleteItemI) {
    await api.delete(`${urls.RF.playerWins.delete}/${id}`).then((res) => {
        if (successCallBack) {
            successCallBack(res)
        }
...
async function submitForm(formData: playerWinsI , options?: submitFormOptions) {
    ...
    const url = formData?.id ? `${urls.RF.playerWins.update}/${formData.id}` : urls.RF.playerWins.create;
