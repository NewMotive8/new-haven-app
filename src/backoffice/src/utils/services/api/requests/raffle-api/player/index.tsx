...
async function getItems(params?: pageableProps) {
    return api.get(urls.RF.player.getAll, { params }).then((res) => res.data)
}
...
async function handleConfirmDelete({ id, successCallBack, errorCallBack }: deleteItemI) {
    await api.delete(`${urls.RF.player.delete}/${id}`).then((res) => {
        if (successCallBack) {
            successCallBack(res)
        }
...
async function submitForm(formData: playerI , options?: submitFormOptions) {
    const { errorCallBack, successCallBack, silent } = options || {}
    const action = formData?.id ? api.put : api.post;
    const url = formData?.id ? `${urls.RF.player.update}/${formData.id}` : urls.RF.player.create;
