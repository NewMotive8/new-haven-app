...
async function getItems(params?: pageableProps) {
    return api.get(urls.operators.getAll, { params }).then((res) => ({ content: res.data }))
}
...
async function handleConfirmDelete({ id, successCallBack, errorCallBack }: deleteItemI) {
    await api.delete(`${urls.operators.delete}/${id}`).then((res) => {
        if (successCallBack) {
            successCallBack(res)
        }
...
async function submitForm(formData: operatorsI, options?: submitFormOptions) {
    const { errorCallBack, successCallBack, silent } = options || {}
    const action = formData?.id ? api.put : api.post
    const url = formData?.id ? `${urls.operators.update}/${formData.id}` : urls.operators.create
...
async function generateNewApplicationKey(id: number) {
    return api.get(urls.operators.generateKey.replace('{{operatorId}}', `${id}`))
}
