...
export interface winI {
    ...
    instantWin: string;
  status: string;
  tournamentRaceInstanceId: number;
  players: playerI[];
  createdDate: string;
}
...
const defaultItem: winI = {
    ...
    instantWin: '',
    status: '',
    tournamentRaceInstanceId: 0,
    players: [],
    createdDate: '',
};

async function getItems(params?: pageableProps) {
    return api.get(urls.TR.win.getAll, { params }).then((res) => res.data)
}
...
async function handleConfirmDelete({ id, successCallBack, errorCallBack }: deleteItemI) {
    await api.delete(`${urls.TR.win.delete}/${id}`).then((res) => {
        if (successCallBack) {
            successCallBack(res)
        }
...
async function submitForm(formData: winI , options?: submitFormOptions) {
    ...
    const url = formData?.id ? `${urls.TR.win.update}/${formData.id}` : urls.TR.win.create;
