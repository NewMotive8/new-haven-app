...
export interface tournamentRaceI {
    id?: number | null | undefined
    name: string;
    enabled: boolean;
    brandId: number;
    tournamentRaceInstances?: instanceI[];
}

const defaultItem: tournamentRaceI = {
    id: undefined,
    name: '',
    enabled: false,
    brandId: 0,
    tournamentRaceInstances: [],
}

async function getItems(params?: pageableProps) {
    return api.get(urls.TR.tournament.getAll, { params }).then((res) => res.data)
}
...
async function handleConfirmDelete({ id, successCallBack, errorCallBack }: deleteItemI) {
    await api.delete(`${urls.TR.tournament.delete}/${id}`).then((res) => {
        if (successCallBack) {
            successCallBack(res)
        }
...
async function submitForm(formData: tournamentRaceI, options?: submitFormOptions) {
    const { errorCallBack, successCallBack, silent } = options || {}
    const action = formData?.id ? api.put : api.post;
    const url = formData?.id ? `${urls.TR.tournament.update}/${formData.id}` : urls.TR.tournament.create;
...
export default tournamentRaceApi
