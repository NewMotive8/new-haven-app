...
async function getItems(params?: pageableProps) {
    return api.get(urls.TR.playerWins.getAll, { params }).then((res) => (res.data))
}
...
async function handleConfirmDelete({ id, successCallBack, errorCallBack }: deleteItemI) {
    await api.delete(`${urls.TR.playerWins.delete}/${id}`).then((res) => {
        if (successCallBack) {
            successCallBack(res)
        }
...
async function submitForm(formData: playerWinsI , options?: submitFormOptions) {
    ...
    const url = formData?.id ? `${urls.TR.playerWins.update}/${formData.id}` : urls.TR.playerWins.create;
