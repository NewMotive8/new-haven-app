...
async function getItems(params?: pageableProps) {
    return api.get(urls.TR.player.getAll, { params }).then((res) => res.data)
}
...
async function handleConfirmDelete({ id, successCallBack, errorCallBack }: deleteItemI) {
    await api.delete(`${urls.TR.player.delete}/${id}`).then((res) => {
        if (successCallBack) {
            successCallBack(res)
        }
...
async function submitForm(formData: playerI , options?: submitFormOptions) {
    ...
    const url = formData?.id ? `${urls.TR.player.update}/${formData.id}` : urls.TR.player.create;
