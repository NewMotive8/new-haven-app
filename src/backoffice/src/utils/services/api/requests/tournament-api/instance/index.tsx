...
async function getItems(params?: pageableProps) {
    return api.get(urls.TR.instance.getAll, { params }).then((res) => (res.data))
}
...
async function handleConfirmDelete({ id, successCallBack, errorCallBack }: deleteItemI) {
    await api.delete(`${urls.TR.instance.delete}/${id}`).then((res) => {
        if (successCallBack) {
            successCallBack(res)
        }
...
async function submitForm(formData: instanceI, options?: submitFormOptions) {
    ...
    const url = formData?.id ? `${urls.TR.instance.update}/${formData.id}` : urls.TR.instance.create;
