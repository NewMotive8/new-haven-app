...
async function getPayouts(params?: pageableProps) {
    return api.get(urls.TR.payout.getAll, { params }).then((res) => res.data)
}

async function getPayoutByRace(raceId: number) {
    return api.get(urls.TR.payout.getByRace.replace('{{raceId}}', `${raceId}`)).then((res) => res.data)
}
...
async function handleConfirmDeletePayout({ id, successCallBack, errorCallBack }: deletePayoutI) {
    await api.delete(`${urls.TR.payout.delete}/${id}`).then((res) => {
        if (successCallBack) {
            successCallBack(res)
        }
...
async function submitPayoutForm(formData: payoutI, options?: submitFormOptions) {
    ...
    const url = formData?.id ? `${urls.TR.payout.update}/${formData.id}` : urls.TR.payout.create
