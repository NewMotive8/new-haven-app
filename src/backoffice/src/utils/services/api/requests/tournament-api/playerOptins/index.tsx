...
async function getByPlayer({ playerId, ...params }: GetByPlayerPros) {
  return api
    .get(`${urls.TR.playerOptins.getAll}/player/${playerId}`, { params })
    .then((res) => ({ content: res.data }))
}

async function getItems(params?: pageableProps) {
  return api
    .get(urls.TR.tournament.getAll, { params })
    .then((res) => ({ content: res.data }))
}
...
async function submitForm(
  ...
  const url = formData?.id ? `${urls.TR.playerOptins.update}/${formData.id}` : urls.TR.playerOptins.create
