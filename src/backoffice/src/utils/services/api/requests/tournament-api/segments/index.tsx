...
async function getItems(params?: pageableProps) {
    return api.get(urls.TR.segments.getAll, { params }).then((res) => ({ content: res.data }))
}

async function segmentsPerTournamentRace(tournamentId: number) {
    const res = await api.get(urls.TR.segments.segmentsPerJTournamentRace.replace('{{tournamentId}}', `${tournamentId}`))
    return (res?.data?.content || res?.data || [])
}
async function segmentsPerPlayer(playerId: number) {
    const res = await api.get(urls.TR.segments.segmentsPerPlayer.replace('{{playerId}}', `${playerId}`))
    return (res?.data?.content || res?.data || [])
}
...
async function bindSegmentToTournament({ tournamentId, segmentId }: bindUnbind) {
    return api.get(urls.TR.segments.bind
        .replace('{{tournamentId}}', `${tournamentId}`)
        .replace('{{segmentId}}', `${segmentId}`))
        .then((res) => ({ content: res.data }))
}
async function unbindSegmentToTournament({ tournamentId, segmentId }: bindUnbind) {
    return api.get(urls.TR.segments.unbind
        .replace('{{tournamentId}}', `${tournamentId}`)
        .replace('{{segmentId}}', `${segmentId}`))
        .then((res) => ({ content: res.data }))
}
...
async function bindSegmentToPlayer({ playerId, segmentId }: bindUnbindPlayer) {
    return api.get(urls.TR.segments.bindPlayer
        .replace('{{playerId}}', `${playerId}`)
        .replace('{{segmentId}}', `${segmentId}`))
        .then((res) => ({ content: res.data }))
}
async function unbindSegmentToPlayer({ playerId, segmentId }: bindUnbindPlayer) {
    return api.get(urls.TR.segments.unbindPlayer
        .replace('{{playerId}}', `${playerId}`)
        .replace('{{segmentId}}', `${segmentId}`))
        .then((res) => ({ content: res.data }))
}
...
async function handleConfirmDelete({ id, successCallBack, errorCallBack }: deleteItemI) {
    await api.delete(`${urls.TR.segments.delete}/${id}`).then((res) => {
        if (successCallBack) {
            successCallBack(res)
        }
...
async function submitForm(formData: segmentsI, options?: submitFormOptions) {
    ...
    const url = formData?.id ? `${urls.TR.segments.update}/${formData.id}` : urls.TR.segments.create
...
const segmentsApi = {
    ...
    submitForm,
    getItems,
    segmentsPerTournamentRace,
    segmentsPerPlayer,
    bindSegmentToTournament,
    unbindSegmentToTournament,
    bindSegmentToPlayer,
