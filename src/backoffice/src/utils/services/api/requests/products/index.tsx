...
function getItems(params?: pageableProps) {
    return api.get(urls.products.getAll, { params }).then((res) => ({ content: res.data }))
}
...
async function handleConfirmDelete({ id, successCallBack, errorCallBack }: deleteItemI) {
    await api.delete(`${urls.products.delete}/${id}`).then((res) => {
        if (successCallBack) {
            successCallBack(res)
        }
...
async function submitForm(formData: productsI, options?: submitFormOptions) {
    const { errorCallBack, successCallBack, silent } = options || {}
    const { brand, ...payload } = formData
    const action = formData?.id ? api.put : api.post
    const url = formData?.id ? `${urls.products.update}/${formData.id}` : urls.products.create
    await action(url, payload).then(() => {
        if (successCallBack) {
            successCallBack()
        }
        if (!silent) {
