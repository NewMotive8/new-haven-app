...
export default function EventsTab() {
    ...
    const { data: events, isLoading, refetch } = useQuery(`events-jp-${selectedItem.id}`, () => eventsApi.eventsPerRaffle(selectedItem.id), {})
    ...
    function handleConfirmUnbind(event: eventsI) {
        setLoading(true)
        eventsApi.unbindEventToRaffle({ raffleId: selectedItem.id, eventId: event.id as number }).then(() => {
            toastSuccess(
                <Typography
