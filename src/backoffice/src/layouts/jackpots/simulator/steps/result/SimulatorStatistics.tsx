...
export default function SimulatorStatistics(props: Props) {
  ...
  const averageAmountWon = calculateAverage(
    winAmountCounter,
    winCounter,
    2,
    Infinity,
  )
  const contributionRatio = calculateAverage(
    contributionCounter,
    winAmountCounter,
    6,
    8,
  ...
  const target = (
    poolContribution
    / (simulatorData.averageWinAmount
      || simulatorData.fixedWinAmount
      || simulatorData.maximumWinAmount)
  ).toFixed(6)
  ...
  const isMultiLevel = simulatorData.jackpotType === 'MULTI_LEVEL'
  const tierWins = ((simulatorResponse as any)?.tierWins || {}) as Record<string, number>
            ...
            <Grid gap="1.5rem">
              ...
              >
                <Typography
                  ...
                  weight={600}
                  translateKey="contribution-ratio"
            ...
            content={(
              <Grid gap="0.5rem">
                {Object.keys(tierWins).length === 0 ? (
                  ...
                  Object.entries(tierWins).map(([tier, count]) => (
                    <Grid
                      key={tier}
                      gap="0.25rem"
                      responsiveWidth={{ sm: 100 }}
                      style={{ textAlign: 'left', display: 'flex', justifyContent: 'space-between', flexDirection:'row' }}
                      
                    >
                      <Typography style={{ width: '100%', fontWeight: 600 }}>
                        {`Tier ${tier}`}
                      </Typography>
                      <Typography style={{ width: '100%' }}>{count}</Typography>
                    </Grid>
                  ))
                )}
              </Grid>
        ...
        <InfoCard
          ...
          content={(
            <Typography
              ...
              replaces={[
                {
                  code: '{{winAmountCounter}}',
                  value: `${parseFloat(`${winAmountCounter}`)?.toFixed(2)}`,
                },
                {
                  code: '{{winTarget}}',
                  value: `${
              ...
              defaultContent={(
                <p>
                  In total,
                  <b>{`${parseFloat(`${winAmountCounter}`)?.toFixed(2)}`}</b>
                  was won,
                  <br />
                  making an average of
                  <b>{` ${averageAmountWon} `}</b>
        ...
        <InfoCard
          ...
          content={(
            <Typography
              ...
              replaces={[
                {
                  code: '{{contributionCounter}}',
                  value: `${contributionCounter?.toFixed(2)}`,
                },
              ...
              defaultContent={(
                <p>
                  ...
                  <b>{` ${contributionCounter?.toFixed(2)} `}</b>
                  in contributions,
                  <br />
                  giving a contribution ratio of
                  <b>{` ${contributionRatio}`}</b>
                </p>
              )}
            />
          )}
        ...
        <InfoCard
          ...
          content={(
            <Typography
              ...
              replaces={[{ code: '{{maxWin}}', value: `${maxWin}` }]}
              returnDefault="defaultContent"
              defaultContent={(
                <p>
                  Maximum single
                  <br />
                  amount won:
                  <b>{parseFloat(`${maxWin}`).toFixed(2)}</b>
                </p>
              )}
            />
          )}
