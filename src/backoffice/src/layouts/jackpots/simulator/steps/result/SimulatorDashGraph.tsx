...
function downDataSize(data: WinEvents[], targetLength: number) {
  ...
  for (let i = 0; i < targetLength; i += 1) {
    ...
    const avgWinningAmount = binSlice.reduce((acc, curr) => acc + curr.winningAmount, 0)
      / binSlice.length
    const avgCurrentPoolAmount = binSlice.reduce((acc, curr) => acc + curr.currentPoolAmount, 0)
      / binSlice.length
    const avgCurrentSeedAmount = binSlice.reduce((acc, curr) => acc + curr.currentSeedAmount, 0)
      / binSlice.length
    const representativeWinningIteration = binSlice[binSlice.length - 1].winningIteration

    const avgWinningTier = binSlice.reduce((acc, curr) => acc + (curr.winningTier ?? 0), 0) / binSlice.length
    const representativeWinningTier = binSlice[binSlice.length - 1]?.winningTier ?? avgWinningTier
   ...
   winningTier: representativeWinningTier,
    })
  }
      ...
      >
        ...
        <Grid>
          <Typography
            ...
            style={{ width: '100%'}}
          />
          <Typography color="var(--warn)" weight={600} elementType="p">
            {parseFloat(info?.currentSeedAmount)?.toFixed(2)}
          </Typography>
        </Grid>
        <Grid>
          <Typography
          ...
          <Grid>
            ...
            <Grid gap="0.5rem" horizontalAlgin="center">
              <Button
                ...
                onClick={() => {
                  setLines({ ...lines, winningAmount: !lines.winningAmount })
                }}
              >
                <Typography
              ...
              <Button
                ...
                onClick={() => {
                  setLines({
                    ...lines,
                    currentPoolAmount: !lines.currentPoolAmount,
                  })
                }}
              >
                <Typography
              ...
              <Button
                ...
                onClick={() => {
                  setLines({
                    ...lines,
                    currentSeedAmount: !lines.currentSeedAmount,
                  })
                }}
              >
                <Typography
