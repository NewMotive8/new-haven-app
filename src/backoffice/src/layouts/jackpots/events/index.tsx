...
import eventsApi, { eventsI } from 'utils/services/api/requests/events'
import ListCrud from './list'
import EventsForm from './form'
import EventsCrudHeader from './header'
...
export default function EventsCrud() {
    ...
    const [selectedItem, setSelectedItem] = useState<eventsI | null>(null)
    ...
    function submitForm() {
        setLoading(true)
        eventsApi.submitForm(selectedItem as eventsI, {
            successCallBack: () => {
                askRefresh()
                setSelectedItem(null)
                setLoading(false)
