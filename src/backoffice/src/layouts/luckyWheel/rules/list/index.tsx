...
import { LWRuleApi } from 'utils/services/api/requests/luckWheel/rule';
import { LWRulesContext } from '..';
import { columns } from './listSettings';

export default function ListCrud() {
  const { setSelectedItem, listRules } = useContext(LWRulesContext);
  return (
    <>
      <Grid horizontalAlgin='flex-end' margin='mb-3'>
        <Button
          id='add-item-button'
          onClick={() => {
            setSelectedItem(LWRuleApi.defaultRule);
          }}
          color='primary'
        >
          <Grid
