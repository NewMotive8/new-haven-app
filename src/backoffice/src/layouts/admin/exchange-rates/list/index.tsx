...
import exchangeRatesApi from 'utils/services/api/requests/exchangeRates'
    ...
    <>
      <Grid horizontalAlgin="flex-end" margin="mb-3">
        <Button
          id="add-item-button"
          onClick={() => {
            setSelectedItem(exchangeRatesApi.defaultItem)
          }}
          color="primary"
        >
          <Grid
      ...
      <DataGridV3
        ...
        columns={columns}
        dataService={(p) => exchangeRatesApi.getItems(p)}
        dataGridId={`exchange-rates-${refreshState}`}
      />
    </>
  )
}
