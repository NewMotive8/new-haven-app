        ...
        <Grid gap="1rem" verticalAlgin='flex-start'>
            ...
            <Grid responsiveWidth={{ sm: 100, md: 'calc(50% - 0.5rem)' }}>
                <InputGroup
                    ...
                    id="tournamentRaceInstanceId"
                    name="tournamentRaceInstanceId"
                    label="tournamentRaceInstanceId"
                    feedback={<Typography {...errors?.tournamentRaceInstanceId} />}
                    status={errors?.tournamentRaceInstanceId && 'error'}
                    value={selectedItem?.tournamentRaceInstanceId}
                    onChange={({ target }) => { updateField(target.name, target.value) }}
                />
            </Grid>
            <Grid responsiveWidth={{ sm: 100, md: 'calc(50% - 0.5rem)' }}>
