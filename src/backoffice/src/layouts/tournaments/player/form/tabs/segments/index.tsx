...
export default function SegmentsTab() {
    ...
    const { data: segments, isLoading, refetch } = useQuery(`trr-segments-player-${selectedItem.id}`, () => segmentsApi.segmentsPerPlayer(selectedItem.id), {})
    ...
    function handleDisplayDialog() {
        displayDialog({
            ...
            content: (<AddNewSegment currentPlayer={selectedItem} />),
    ...
    function handleConfirmUnbind(segment: segmentsI) {
        confirmBox({
            confirmMessage: (
                <Typography
                    translateGroup="trr-player-segment"
                    translateKey="are-you-sure-in-remove-this-segment"
                />
            ),
            onConfirm: () => {
                setLoading(true)
                segmentsApi.unbindSegmentToPlayer({ playerId: selectedItem.id, segmentId: segment.id as number }).then(() => {
                    toastSuccess(
                        <Typography
                            translateGroup="trr-player-segment"
        ...
        <Grid gap="0.5rem">
            <Grid>
                <Button id="add-segment-cta" onClick={() => handleDisplayDialog()}>
                    <Typography
                        translateGroup="player-segments"
