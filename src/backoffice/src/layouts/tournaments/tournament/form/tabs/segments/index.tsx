...
export default function SegmentsTab() {
    ...
    const { data: segments, isLoading, refetch } = useQuery(`segments-trr-${selectedItem.id}`, () => segmentsApi.segmentsPerTournamentRace(selectedItem.id), {})
    ...
    function handleDisplayDialog() {
        displayDialog({
            ...
            content: (<AddNewSegment currentTournament={selectedItem} />),
    ...
    function handleConfirmUnbind(segment: segmentsI) {
        confirmBox({
            ...
            onConfirm: () => {
                setLoading(true)
                segmentsApi.unbindSegmentToTournament({ tournamentId: selectedItem.id, segmentId: segment.id as number }).then(() => {
                    toastSuccess(
                        <Typography
                            translateGroup="tournament-race-segment"
