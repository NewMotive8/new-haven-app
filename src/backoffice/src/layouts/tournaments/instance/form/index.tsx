...
export default function InstanceForm() {
    ...
    return (
        ...
        >
            <form  className="mt-5">
                <Grid wrap={!!tournamentId ? 'wrap' :largeApp ? 'nowrap' : 'wrap'} verticalAlgin='stretch' gap="0.5rem">
                    {formTabs.length > 1 && (
                        <Grid responsiveWidth={{ sm: 100, lg: !!tournamentId ? 100 : '300px', xl: !!tournamentId ? 100 : '400px' }}>
                            <TabSelector
                                currentTab={currentTab}
                                setCurrentTab={setCurrentTab}
                                tabs={formTabs}
                                orientation={tournamentId ? 'horizontal' : largeApp ? 'vertical' : 'horizontal'}
                            />
                        </Grid>
                    )}
                    <Grid>
