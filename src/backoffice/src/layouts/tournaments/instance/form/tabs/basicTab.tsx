...
export default function BasicTab() {
  const { selectedItem, tournamentId } = React.useContext(CrudContext);
    ...
    <Grid gap="1rem" verticalAlgin="flex-start">
      <Grid>
        <InputGroup
          id="name"
          name="name"
          label="tournament-race-instance-name"
      ...
      <Grid>
        <Grid hidden={!!tournamentId}>
          <GenericSelector
            columns={["id", "name"]}
            onSelect={(value) => updateField("tournamentId", value.id)}
            label="tournament race id"
            mainlyKey="name"
            dataService={(p) => tournamentRaceApi.getItems(p)}
            dataServiceId="tournament-race"
            value={selectedItem.tournamentId}
          />
        </Grid>
      </Grid>
      <Grid>
