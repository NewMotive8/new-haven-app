...
export default function ListCrud() {
  const { refreshState, setSelectedItem, spinSprintId } =
    useContext(CrudContext)

  const createNewWidget = () => {
    if (typeof spinSprintId !== 'number') return

    setSelectedItem({
      ...widgetApi.defaultItem,
      spinSprint: { id: spinSprintId },
    })
  }

  if (typeof spinSprintId !== 'number') {
    return null
  }
  ...
  columns={columns}
  dataService={async () => {
    const widgets = await widgetApi.getBySpinSprint(spinSprintId)
  ...
  dataGridId={`widget-list-${spinSprintId}-${refreshState}`}
  enablePagination
/>
