...
import playerOptinsApi, { playerOptinsI } from 'utils/services/api/requests/jackpot-race-api/playerOptins'
...
export default function OptInTab() {
  ...
  const { data: jackpots, refetch: jackpotRefetch } = useQuery('jackpot-races', () => jackpotRaceApi.getItems())
        ...
        >
          <Grid margin="mb-5" horizontalAlgin="center">
            <Typography
              weight={700}
              size="xl"
              translateGroup="jackpot-race-opt-in-Tab-dialog-alert"
  ...
  const spinSprintIds = new Set(
    playerOption?.content.map((item: any) => item.spinSprintId),
  )
  const filtroJackpot = playerOption?.content.map((item: any) => {
    if (spinSprintIds.has(item.spinSprintId)) {
      const matchingItem = jackpots?.content.find(
        (element: any) => element.id === item.spinSprintId,
      )
      return {
        playerOption: item,
        ...matchingItem,
  ...
  const filtroExitJackpot = jackpots?.content.filter(
    (item: any) => !spinSprintIds.has(item.id),
  )
  return (
    <Grid gap="0.5rem">
      <Grid responsiveWidth={{ sm: 100, md: 'calc(100% - 0.25rem)' }}>
        <Grid margin="mb-2">
          <Typography
            weight={700}
            size="md"
            translateGroup="jackpot-race-opt-in-Tab-table-title"
        ...
        <Grid margin={['mb-2', 'mt-5']}>
          <Typography
            ...
            size="md"
            translateGroup="jackpot-race-opt-in-Tab-table-title"
            translateKey="Jackpot race never opted before"
          />
        </Grid>
        <DataGridV2
