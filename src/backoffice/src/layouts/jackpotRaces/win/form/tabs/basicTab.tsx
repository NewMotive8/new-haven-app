...
export default function BasicTab() {
    ...
    return (
        <Grid gap="1rem" verticalAlgin='flex-start'>
            ...
            <Grid responsiveWidth={{ sm: 100, md: 'calc(50% - 0.5rem)' }}>
                <InputGroup
                    id="jackpotRaceInstanceId"
                    name="jackpotRaceInstanceId"
                    label="jackpotRaceInstanceId"
                    feedback={<Typography {...errors?.jackpotRaceInstanceId} />}
                    status={errors?.jackpotRaceInstanceId && 'error'}
                    value={selectedItem?.jackpotRaceInstanceId}
                    onChange={({ target }) => { updateField(target.name, target.value) }}
                />
            </Grid>
            <Grid responsiveWidth={{ sm: 100, md: 'calc(50% - 0.5rem)' }}>
