...
export default function SegmentsTab() {
    ...
    const { data: segments, isLoading, refetch } = useQuery(`segments-jpr-${selectedItem.id}`, () => segmentsApi.segmentsPerJackpotRace(selectedItem.id), {})
    ...
    function handleConfirmUnbind(segment: segmentsI) {
        confirmBox({
            confirmMessage: (
                <Typography
                    translateGroup="jackpot-race-segment"
                    translateKey="are-you-sure-in-remove-this-segment"
                />
            ),
            onConfirm: () => {
                setLoading(true)
                segmentsApi.unbindSegmentToJackpot({ spinSprintId: selectedItem.id, segmentId: segment.id as number }).then(() => {
                    toastSuccess(
                        <Typography
                            translateGroup="jackpot-race-segment"
        ...
        <Grid gap="0.5rem">
            <Grid>
                <Button id="add-segment-cta" onClick={() => handleDisplayDialog()}>
                    <Typography
                        translateGroup="jackpot-race-segments"
