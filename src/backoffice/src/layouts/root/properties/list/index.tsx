...
import propertiesApi from 'utils/services/api/requests/properties'
    ...
    <>
      <Grid horizontalAlgin="flex-end" margin="mb-3">
        <Button
          id="add-item-button"
          onClick={() => {
            setSelectedItem(propertiesApi.defaultItem)
          }}
          color="primary"
        >
          <Grid
      ...
      <DataGridV3
        ...
        columns={columns}
        dataService={(p) => propertiesApi.getItems(p)}
        dataGridId={`properties-${refreshState}`}
      />
    </>
  )
}
