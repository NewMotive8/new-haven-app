...
import PlanInfoTab from './tabs/planInfo'

interface FormContextI {
    updateField: (fieldName: string, value: any, parentKey?: string) => void
...
export const FormContext = React.createContext<FormContextI>({
    updateField: (fieldName: string, value: any, parentKey?: string) => {},
...
export default function OperatorsForm() {
    ...
    function updateField(fieldName: string, value: any, parentKey?: string) {
        setErrors((d: any) => ({ ...d, [fieldName]: '' }))
        setSelectedItem((d: any) => {
            const updated = { ...d, [fieldName]: value }
            return updated
        })
    }
    ...
    const formTabs: Array<TabSelectorTabI> = [
        {
            value: 'basic',
            label: <Typography translateGroup="forms-tabs-label" translateKey="operators-info" />,
            content: <BasicTab />,
        },
        {
            value: 'planInfo',
            label: <Typography translateGroup="forms-tabs-label" translateKey="operators-planInfo" />,
            content: <PlanInfoTab />,
        },
    ]

    const tabsHelper: { [key in any]: React.ReactNode } = {
        basic: textTranslated({ group: 'forms-tabs-helpers', key: 'operators-basic-tab-help', returnDefault: 'nothing' }),
        planInfo: textTranslated({ group: 'forms-tabs-helpers', key: 'operators-planInfo-tab-help', returnDefault: 'nothing' }),
    }
