...
import tiersApi from 'utils/services/api/requests/tiers'
    ...
    return (
        <>
            <Grid horizontalAlgin="flex-end" margin="mb-3">
                <Button id="add-item-button" onClick={() => { setSelectedItem(tiersApi.defaultItem) }} color="primary">
            ...
            <DataGridV3
                ...
                columns={columns}
                dataService={(p) => tiersApi.getItems(p)}
                dataGridId={`tiers-${refreshState}`}
            />
        </>
    )
}
