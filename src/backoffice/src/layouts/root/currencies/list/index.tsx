...
import currenciesApi from 'utils/services/api/requests/currencies'
    ...
    return (
        <>
            <Grid horizontalAlgin="flex-end" margin="mb-3">
                <Button id="add-item-button" onClick={() => { setSelectedItem(currenciesApi.defaultItem) }} color="primary">
            ...
            <DataGridV3
                ...
                columns={columns}
                dataService={(p) => currenciesApi.getItems(p)}
                dataGridId={`currencies-${refreshState}`}
                enablePagination
            />
        </>
    )
