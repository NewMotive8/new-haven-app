 ...
 */
function sanitizeIncomingDraft(
  ...
  if (draft.type === 'frequency') {
    ...
    const cf = tryParse(draft.contributionFrequency);
    const wf = tryParse(draft.winFrequency);

    const pickInterval = (v: unknown): 'DAILY' | 'WEEKLY' | 'MONTHLY' | undefined => {
      if (v === 'DAILY' || v === 'WEEKLY' || v === 'MONTHLY') return v;
      return undefined;
    };
    const pickTime = (...vals: unknown[]): string | undefined => {
    ...
    if (draft.freqInterval == null) {
      draft.freqInterval =
        pickInterval(cf?.frequency) ?? pickInterval(wf?.frequency) ?? 'DAILY';
    }
    ...
    // Rebuild clone toggle from whether the two windows are identical.
    if (draft.cloneContribToWin == null) {
      const cs = draft.contribStartTime, ce = draft.contribEndTime;
      const ws = draft.winStartTime ?? cs, we = draft.winEndTime ?? ce;
  ...
  const effContribType =
    draft.contributionMode === 'split' ? draft.totalContributionType : draft.contributionType;
